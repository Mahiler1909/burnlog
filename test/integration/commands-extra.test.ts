...
describe("branchCommand", () => {
  ...
  it("compares two branches in JSON format", async () => {
    ...
    const { branchCommand } = await import("../../src/cli/commands/branch.js");
    await branchCommand(["feature/a", "feature/b"], { format: "json" });
  ...
  it("prints message when no sessions found for comparison", async () => {
    clearTestSessions();
    const { branchCommand } = await import("../../src/cli/commands/branch.js");
    await branchCommand(["a", "b"], { format: "json" });

    const output = consoleSpy.mock.calls.map((c: unknown[]) => c[0]).join("");
    expect(output).toContain("No sessions found");
  });

  it("rejects more than 2 branches", async () => {
    setTestSessions([createSession()]);
    const { branchCommand } = await import("../../src/cli/commands/branch.js");
    await branchCommand(["a", "b", "c"], { format: "json" });

    const output = consoleSpy.mock.calls.map((c: unknown[]) => c[0]).join("");
    expect(output).toContain("Compare supports exactly 2 branches");
  });
});
